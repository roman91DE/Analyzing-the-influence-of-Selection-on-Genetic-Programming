...
\item
  ...
  performance on both the training as well as the testing dataset than
  \(\epsilon\)-Lexicase selection. These findings contradict the
  hypothesis \(H0_{1}\) descibed in subsection 3.1 and are contrary to
...
To further examine the generalization behaviour of both selection
...
small sign of overfitting can be interpreted as a weak indicator for an
inferior generalization behaviour of \(\epsilon\)-Lexicase in comparison
...
\(\epsilon\)-Lexicase selection for the choosen symbolic regression
task. It can be observed that at all stages of the evolution tournament
selection based GP achieves a lower mean error for both datasets.
...
\includegraphics{./misc/erklärung.pdf}\\
